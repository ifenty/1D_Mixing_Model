...
% Allow bare "_" in prose (filenames/variables) to print literally; still a
% subscript in math mode. Must load after hyperref. Preserves underscores in
% \label/\ref/\cite and in \verb / listings.
...
% ============================================================
\section{Governing Equations (overview)}
...
\subsection{TKE Evolution Equation}

The fundamental prognostic equation solved by GGL90 is the turbulent kinetic energy (TKE) evolution equation:

\begin{equation}
\frac{\partial \TKE}{\partial t} + \vec{u} \cdot \nabla \TKE = P + B - \varepsilon + \frac{\partial}{\partial z}\left(\KE \frac{\partial \TKE}{\partial z}\right)
\end{equation}

where:
\begin{itemize}
\item $P = \KM \Ssq$ --- Shear production from vertical velocity gradients
\item $B = -\KH \Nsq$ --- Buoyancy flux (positive for unstable stratification)
\item $\varepsilon = c_\varepsilon \frac{\TKE^{3/2}}{L}$ --- Dissipation rate
\item $\KE = \KM$ --- Vertical diffusivity of TKE
\end{itemize}

The TKE field $\TKE(x,y,z,t)$ is the only prognostic variable in GGL90; all mixing coefficients are diagnosed from TKE at each time step.

\subsection{Eddy Viscosity and Diffusivity}

The eddy viscosity $\KM$ and diffusivity $\KH$ are computed from TKE via:

\begin{equation}
\KM(z) = c_k L(z) \sqrt{\TKE(z)}
\end{equation}

\begin{equation}
\KH(z) = \frac{\KM(z)}{\alpha}
\end{equation}

where:
\begin{itemize}
\item $c_k = 0.1$ --- Dimensionless closure constant
\item $L(z)$ --- Mixing length (stratification-dependent, bounded by distance to boundaries)
\item $\alpha$ --- Turbulent Prandtl number (ratio $\KM/\KH$; default 1.0, ECCOv4: 30.0)
\end{itemize}
...
\begin{equation}
L(z) = \sqrt{2} \frac{\sqrt{\TKE(z)}}{\sqrt{\max(\Nsq(z), 0)}}
\quad \text{(stable stratification)}
\end{equation}

with hard limits imposed by distance to boundaries:

\begin{equation}
L(z) \leftarrow \min\left(\max\left(L(z), L_{\min}\right), \kappa |z|\right)
\end{equation}
...
\begin{equation}
\TKE(z=0) = \max\left(\frac{u_*^2}{B_1}, \text{GGL90TKEsurfMin}\right)
\end{equation}
...
\begin{equation}
\TKE(z=z_{\text{bottom}}) = \max\left(\text{GGL90TKEbottom}, \text{GGL90TKEmin}\right)
\end{equation}
...
See \S\ref{sec:main_driver} for the complete computational sequence and \S\ref{sec:core_update} for detailed time-stepping procedures.
...
% ============================================================
\section{Main Driver Routine}
\label{sec:main_driver}
...
% ============================================================
\section{Stratification and Buoyancy}
...
% ------------------------------------------------------------
\section{Interior Mixing}
\label{sec:interior_mixing}
...
\textbf{Correction (2026-09-16, 1DMIX-014):} an earlier revision of this section claimed
\verb|GGL90mixingLengthMin| ``effectively sets a minimum mixing length everywhere, including
at the bottom.'' That claim is \emph{only} true for \verb|mxlMaxFlag| $\in \{0,1,2\}$. It is
\textbf{false} for \verb|mxlMaxFlag|~=~3 (this project's ECCOv4r4-documented default), which
does not floor $\ell$ itself at all -- see the corrected code listing below. This exact
documentation error led to a real Python-port bug: a blanket \texttt{max($\ell$,~$\ell_{\min}$)}
was applied to every level for every \verb|mxlMaxFlag|, clamping real (sub-minimum) mixing
lengths back up to $\ell_{\min}$ for the \verb|mxlMaxFlag|~=~3 case. Confirmed against a real
MITgcm \texttt{vermix} capture: \verb|mixing_length[k=0..3]=[3.0, 0.760, 0.112, 0.112]| with
\verb|GGL90mixingLengthMin=3.0| -- levels 1--3 are legitimately below the ``minimum''. Fixed in
the Python port at \verb|1D_Mixing_Model/GGL90/ggl90_scheme_specific.py::GGL90MixingLength.compute|.

\subsubsection{Regularization: \texttt{GGL90\_REGULARIZE\_MIXINGLENGTH}}

The final mixing-length-and-reciprocal step (\verb|ggl90_mixinglength.F| lines 381--416)
branches on \verb|mxlMaxFlag| itself, not just on the regularization compile flag:

\begin{lstlisting}[language=Fortran,caption={(ggl90\_mixinglength.F:381--416) Final mixing length and reciprocal, verified against the real MITgcm source}]
...
C         NOTE: GGL90mixingLength(i,j,k) is NOT reassigned in this branch --
C         it keeps its raw (possibly sub-minimum) two-way-sweep value.
         ENDDO
        ENDDO
       ENDDO
      ELSE
...
For \verb|mxlMaxFlag| $\in \{0,1,2\}$ (the \texttt{ELSE} branch), $\ell$ and its reciprocal are
both floored by the familiar $\ell_{\text{final}} = \max(\ell, \ell_{\text{min}})$ (or, with
\verb|GGL90_REGULARIZE_MIXINGLENGTH|, $\ell_{\text{final}} = \sqrt{\ell^2 + \ell_{\text{min}}^2}$,
a smoother transition as $\ell \to 0$ that can improve adjoint convergence, see
Appendix~\ref{app:adjoint_considerations}). For \verb|mxlMaxFlag|~=~3, $\ell$ itself is left
unfloored; only the reciprocal is regularized, using the geometric mean of $\ell(k)$ and the
downward-sweep companion array \verb|mxLength_Dn(k)| (Section~\ref{sec:mxl_method2}) rather than
$\ell(k)$ alone.
...
\item Regardless of this flag, remember \verb|mxlMaxFlag|~=~3 never floors $\ell$ itself -- only \verb|mxlMaxFlag| $\in \{0,1,2\}$ do.
\end{itemize}
...
% ============================================================
\section{Core Diffusivity/Prognostic Update}
\label{sec:core_update}
...
\subsection{TKE Evolution and Budget}
...
% ============================================================
\subsection{Eddy Coefficient Calculation}
...
\toprule
$\alpha$ & \textbf{TKE Roughness} & \textbf{Normalized} & \textbf{Oscillation} & \textbf{Under-resolved} & \textbf{Status} \\
 & $\sum |\partial^2 \TKE/\partial z^2|$ & \textbf{Roughness} & \textbf{Count} & \textbf{fraction} ($L<2\Delta z$) &  \\
\midrule
1.0  & $1.22 \times 10^{-2}$ & 1.961 & 11 & 0.53 & \textcolor{red}{OSCILLATORY} \\
3.0  & $4.53 \times 10^{-3}$ & 0.959 & 9  & 0.27 & \textcolor{red}{OSCILLATORY} \\
5.0  & $4.61 \times 10^{-4}$ & 0.128 & 3  & 0.06 & \textcolor{orange}{THRESHOLD} \\
7.5  & $4.12 \times 10^{-4}$ & 0.112 & 3  & 0.06 & \textcolor{green!60!black}{SMOOTH} \\
10.0 & $3.82 \times 10^{-4}$ & 0.103 & 3  & 0.12 & \textcolor{green!60!black}{SMOOTH} \\
15.0 & $3.40 \times 10^{-4}$ & 0.090 & 3  & 0.12 & \textcolor{green!60!black}{SMOOTH} \\
20.0 & $3.09 \times 10^{-4}$ & 0.081 & 1  & 0.12 & \textcolor{green!60!black}{SMOOTH} \\
30.0 & $2.61 \times 10^{-4}$ & 0.068 & 1  & 0.12 & \textcolor{green!60!black}{SMOOTH} \\
50.0 & $2.15 \times 10^{-4}$ & 0.055 & 1  & 0.12 & \textcolor{green!60!black}{SMOOTH} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding:} Three independent metrics agree on a threshold of \textbf{$\alpha \geq 5$}. TKE roughness drops by a factor of $\sim$27 from $\alpha=1$ to $\alpha=5$ (falling below $10\%$ of its maximum); the oscillation count falls from 11 to 3; and the fraction of layers whose gradient length scale $L=|\TKE|/|\partial \TKE/\partial z|$ is under-resolved ($L<2\Delta z$) drops from $0.53$ to $0.06$. The last metric directly confirms the mechanism: at $\alpha=1$ roughly half the column is under-resolved, producing the cell-to-cell oscillations, whereas by $\alpha=5$ almost the entire column is well resolved.

\textit{Note on metric choice:} the resolution ratio is reported here as the \emph{median} $L/\Delta z$ together with the \emph{fraction} of under-resolved layers. A single minimum $L/\Delta z$ is not informative because a TKE profile spans many orders of magnitude, so its steepest single transition drives the minimum toward zero at every $\alpha$; the median and the under-resolved fraction are the robust measures.
...
\textbf{Grid dependence:} The minimum $\alpha$ is set by how well the grid resolves the TKE gradient length scale $L$, not by a stability (grid-Reynolds) limit --- the implicit solve is unconditionally stable. What matters is the ratio $L/\Delta z$: since larger $\alpha$ increases $\kappa_h$ and spreads TKE (increasing $L$), the threshold depends on the grid and forcing, so re-testing is recommended for a new configuration.
...
\coordinate (momtop) at ($(mom.north) + (1.5cm,0)$);
\draw[arrow] (momtop) -- node[right, datalabel] {$\tau_x$, $\tau_y$\\(wind stress)} (momtop |- ggl90.north) -- (ggl90.north);
